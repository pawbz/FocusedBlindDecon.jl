...
"""
x is the autocorrelation sequence
"""
function ForceAutoCorr(x::Vector{Float64}, np2)
	!isodd(length(x)) && error("length of x should be odd")
	T=Float64
	nrfft=div(np2,2)+1
	...
	xpad=zeros(np2)
	ypad=zeros(np2)
	nlag=div(length(x)-1,2)
	Conv.pad_truncate!(x, xpad, nlag, nlag, np2, 1)

	xfreq=complex.(zeros(T,nrfft))
	yfreq=complex.(zeros(T,nrfft))
	A_mul_B!(xfreq, fftp, xpad)
	xabs=sqrt.(abs.(xfreq))

	return ForceAutoCorr(xabs, yfreq, ypad, fftp, ifftp, np2)

end

function project!(yout, y, pa=ForceAutoCorr(y))
	T=Float64
	...
	pa.ypad[:]=T(0)
	Conv.pad_truncate!(y, pa.ypad, 0, length(y)-1, pa.np2, 1)
	A_mul_B!(pa.yfreq, pa.fftp, pa.ypad)
	for i in eachindex(pa.xabs)
		pa.yfreq[i]=pa.yfreq[i]*inv(abs(pa.yfreq[i]))*pa.xabs[i]
	end
	A_mul_B!(pa.ypad, pa.ifftp, pa.yfreq)
	Conv.pad_truncate!(yout, pa.ypad, 0, length(y)-1, pa.np2, -1)
	return yout
end
